...
4. **בדוק `nutrition_goal_audit`**. יעד `valid` נשאר ללא שינוי. עבור
   `missing/stale/inconsistent`, דרוש מ-`nutrition-coach` את Calculation Output
   Contract, שמור רק תוצאה `calculated` דרך `scripts/set_goals.py --user-id`,
   בנה packet מחדש ואמת exact read-back ו-`nutrition_goal_status=valid`. אם
   חסרים inputs או נדרש medical review, אל תשמור יעד וכתוב את החסם במפורש.
5. **התייעצות עם 3 agents מומחים במקביל** — העבר לכל אחד את אותו Context Packet:
...
6. **סינתזה** — health-analyst מאחד כפילויות וסתירות. ברירת המחדל היא 3–5 המלצות; עד 7 רק כשיש הצדקה ברורה.
7. **דו"ח אדפטיבי בעברית**:
   - השבוע במשפט אחד
...
8. **כתיבה** ל-`data/users/<user-id>/reports/YYYY-MM-DD.md`.
9. **פרסום idempotent** למשתמש הנכון:
...
10. **שאלות** למשתמש על מידע חסר או אנומליות, בלי לעכב את כתיבת הדו"ח.
