...
class TestAnalyzeImage:
    def test_returns_meal_entries(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            result = analyze_food_image(b"fake-image-bytes")
    ...
    def test_empty_plate_returns_empty(self) -> None:
        mock_client = MagicMock()
        mock_client.chat.completions.create = MagicMock(
...
class TestParseFoodText:
    def test_hebrew(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            result = parse_food_text("אכלתי 2 תפוחים")
    ...
    def test_english(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            result = parse_food_text("I ate 2 apples and a protein shake")
    ...
    def test_mixed_language(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            result = parse_food_text("אכלתי protein shake")

        assert len(result) >= 1

    def test_with_quantities(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            result = parse_food_text("3 eggs scrambled")

        assert len(result) == 1
        assert result[0].calories == 78

    def test_no_quantities_assumes_one_serving(self) -> None:
    ...
    def test_timeout_raises(self) -> None:
        ...
        with patch("shared.vision._get_openai_client", return_value=mock_client):
            with pytest.raises(TimeoutError):
                analyze_food_image(b"fake-image-bytes")
